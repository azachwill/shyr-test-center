...
This article describes five patterns to use with Shiny's [action buttons](/r/reference/shiny/latest/actionButton.html) and [action links](/r/reference/shiny/latest/actionButton.html). Action buttons and action links are different from other Shiny widgets because they are intended to be used exclusively with `observeEvent()` or `eventReactive()`.
...
![](action-button.png ){fig-alt="Image of an action button"}

An action link appears as a hyperlink, but behaves in the same way as an action button.

![](action-link.png){fig-alt="Image of an action link"}
...
In the code above, `session$setCustomMessage()` generates a popup message. `tags$head(tags$script(src = "message-handler.js"))` supplies the JavaScript that makes this possible. See [this example](r/gallery/advanced-shiny/server-to-client-custom-messages/) to learn more about`sendCustomMessage()`.
...
These issues include the following: A submit button has a global scope, so there is no way of specifying which inputs should be on hold and which shouldn't. If an app has two submit buttons, both of them will control the entire app, which is probably not what you want. Also, dynamically created submit buttons (for example, with [`renderUI()`](/r/reference/shiny/latest/renderUI.html) or [`insertUI()`](/r/reference/shiny/latest/insertUI.html)) will not work. 

### Translating `submitButton` code into `actionButton` code

Consider the app below, taken from the documentation for [`submitButton()`](/r/reference/shiny/latest/submitButton.html):
