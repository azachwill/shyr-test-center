...
From time to time Google changes how they do content tracking. The following information is current as of August 2023. If you find that the information is out of date at any point, please let us know on [RStudio Community](https://community.rstudio.com/c/shiny).
...
![](GA-sunshine.png){fig-alt="Shiny app of Sunlight in the US showing percent of time sunny for each state in the United States."}
...
![](GA-signup.png){fig-alt="Screen shot of the sign up page for Google analytics."}
...
![](GA-property.png){fig-alt="Screen shot of the Admin tab in Google analytics sign up."}
...
![](GA-property2.png){fig-alt="Screen shot of property details in Google analytics sign up page."}
...
![](GA-tracking.png){fig-alt="Screen shot of Website tracking section of Google analytics sign up page."}
...
This is very easy to do if you build your Shiny app around an HTML file, as described in [Build your entire UI with HTML](../html-ui).
...
For example, Google Analytics has given us the script below to embed in the Sunshine app. Note that your Google tag will be different from ours, which starts with `GT-` and is used in two places below. The [Google tag is an identifier](https://developers.google.com/tag-platform/gtagjs) for the content you created on Google Analytics.

````{.html filename="google-analytics.html"}
...
![](GA-directory.png){fig-alt="File navigation viewer highlighting the file google-analytics.html"}
...
For example, you can track when users click on a specific link by having the link's `onClick` attribute call `gtag`:
...
````{.js filename="google-analytics.html"}
<!-- Global site tag (gtag.js) - Google Analytics -->
<script async src="https://www.googletagmanager.com/gtag/js?id=GT-XXXXXX"></script>
<script>
  window.dataLayer = window.dataLayer || [];
...
I'll need to redeploy the app to https://shinyapps.io to make sure the hosted version of the app uses this code.
