...
* [Using `bindCache()`](#using-bindcache): [`bindCache()`](/r/reference/shiny/latest/bindCache.html) can be used with `reactive()` and `render` functions, like `renderPlot()` and `renderText()`.
...
Using [`bindCache()`](/r/reference/shiny/latest/bindCache.html) is simple: just pass your `reactive()` or `render` function to `bindCache()`. For example, if this is your reactive expression:
...
This behavior is controlled by the `sizePolicy` parameter -- it is a function that takes two numbers (the actual dimensions of the div) and returns two numbers (the dimensions of the plot that will be rendered). If you want to use a different strategy, you can pass in a different function. See [here](/r/reference/shiny/latest/bindCache.html#caching-with-renderplot-) for more information.
...
[Posit Connect](https://www.rstudio.com/products/connect/) includes support for running multiple R process for an application. For applications deployed to Posit Connect, the best practice is to use a disk cache located in the application directory. To do so, add a line like this to your application:
...
:::{.text-center .my-5}
![](plot-cache-deploy-ignore.png){width="400" fig-alt="Screen shot for Publish to Server"}
:::
...
/r/reference/shiny/latest/bindCache.html#developing-render-functions-for-caching) for more information about this.
