...
![](scaling-and-tuning1.png){fig-alt="Architecture of how application, instance, worker and Connections (end users) relate to each other, in the shape of a pyramid. Application is at the top. Two arrows go down to two instances. Two or three arrows go down from each Instance to Workers. From each Worker 2 to 3 arrows go down to Connections (end users)"}
...
![](scaling-and-tuning2.png){fig-alt="Lifecycle of an applicatoin. Starts with one application and one user and one set of connections. Progresses to multiple end users and therefore more instances and workers. Then decreases again as workers disconnect to return to one application and one worker."}

1. Publisher creates a new application and deploys it to shinyapps.io at [https://www.shinyapps.io/](https://www.shinyapps.io/)
