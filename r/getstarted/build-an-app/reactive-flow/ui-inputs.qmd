...
The `req()` function is the simplest and best way to do this, it ensures that values are available ("[truthy](https://shiny.posit.co/reference/shiny/latest/req.html)") before proceeding with a calculation or action. If any of the given values is not truthy, the operation is stopped by raising a "silent" exception (not logged by Shiny, nor displayed in the Shiny app's UI).
